...
public:
    Bundle(void)
        : mCmd(kCommandUnknown), mVerbose(false), mAndroidList(false),
          ...
          mRClassDir(NULL), mResourceIntermediatesDir(NULL), mSupportAnyDensity(false), mManifestMinSdkVersion(NULL),
          mMinSdkVersion(NULL), mTargetSdkVersion(NULL), mMaxSdkVersion(NULL),
          mVersionCode(NULL), mVersionName(NULL), mPackageName(NULL), mCustomPackage(NULL), mExtraPackages(NULL),
	...
	const android::Vector<android::String8>& getApplicationLabels() const {
		return mApplicationLabels;
	}

	void setApplicationLabels(
			const android::Vector<android::String8>& applicationLabels) {
		mApplicationLabels = applicationLabels;
	}

	const android::Vector<const char*>& getApplicationIcons() const {
		return mApplicationIcons;
	}

	void setApplicationIcons(
			const android::Vector<const char*>& applicationIcons) {
		mApplicationIcons = applicationIcons;
	}

	const android::Vector<int>& getDensities() const {
		return mDensities;
	}

	void setDensities(const android::Vector<int>& densities) {
		mDensities = densities;
	}

	const android::Vector<const char*>& getNativeCodes() const {
		return mNativeCodes;
	}

	void setNativeCodes(const android::Vector<const char*>& nativeCodes) {
		mNativeCodes = nativeCodes;
	}

	const android::Vector<const char*>& getLocales() const {
		return mLocales;
	}

	void setLocales(const android::Vector<const char*>& locales) {
		mLocales = locales;
	}

	const android::Vector<const char*>& getSupportScreens() const {
		return mSupportScreens;
	}

	void setSupportScreens(const android::Vector<const char*>& supportScreens) {
		mSupportScreens = supportScreens;
	}

	bool isSupportAnyDensity() const {
		return mSupportAnyDensity;
	}

	void setSupportAnyDensity(bool supportAnyDensity) {
		mSupportAnyDensity = supportAnyDensity;
	}

	const android::Vector<const char*>& getUsesPermissions() const {
		return mUsesPermissions;
	}

	void setUsesPermissions(
			const android::Vector<const char*>& usesPermissions) {
		mUsesPermissions = usesPermissions;
	}

	const android::Vector<const char*>& getUsesFeatures() const {
		return mUsesFeatures;
	}

	void setUsesFeatures(const android::Vector<const char*>& usesFeatures) {
		mUsesFeatures = usesFeatures;
	}
...
private:
    ...
    android::Vector<android::String8> mApplicationLabels;
    android::Vector<const char*> mApplicationIcons;
    android::Vector<int> mDensities;
    android::Vector<const char*> mNativeCodes;
    android::Vector<const char*> mLocales;
    android::Vector<const char*> mSupportScreens;
    android::Vector<const char*> mUsesPermissions;
    android::Vector<const char*> mUsesFeatures;
    bool		mSupportAnyDensity;
